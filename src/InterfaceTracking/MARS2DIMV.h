...
private:
  ...
  Vector<unsigned int> splitLongEdges(
      const VelocityField<2> &v,
      Vector<Point> &pts,
...
template <int Order>
class MARS2DIMV<Order, VectorFunction>
    : public MARS<2, Order, VectorFunction> {
  template <class T>
  using Vector = std::vector<T>;
...
public:
  ...
  MARS2DIMV(TimeIntegrator<2, VectorFunction> *_TI,
            Real hL,
            Real rtiny = 0.1) :
      Base(_TI), chdLenRange(Vec<Real, 1>(rtiny * hL), Vec<Real, 1>(hL)) {}
...
private:
  void discreteFlowMap(const VectorFunction<2> &v,
                       Vector<Point> &pts,
                       Real tn,
                       Real dt);
...
template <int Order>
class MARS2DIMV<Order, VectorOnHypersurface>
...
public:
  ...
  MARS2DIMV(TimeIntegrator<2, VectorOnHypersurface> *_TI,
            Real hL,
            Real rtiny = 0.1) :
      Base(_TI), chdLenRange(Vec<Real, 1>(rtiny * hL), Vec<Real, 1>(hL)) {}
...
private:
  void discreteFlowMap(const VectorOnHypersurface<2> &v,
                       Vector<Point> &pts,
                       Real tn,
                       Real dt);
