...
template <class T, int Dim>
template <class TExpr>
inline Tensor<T, Dim>::Tensor(const TensorExpr<TExpr> &expr) :
    Tensor(expr.box()) {
  *this = expr;
}
...
struct TensorBinaryOp : public TensorExpr<TensorBinaryOp<TExpr1, TExpr2, Op>> {
  ...
  template <class... Ts>
  auto at(int i, Ts... args) const {
    return Op()(e1.at(i, args...), e2.at(i, args...));
  }
  const auto box() const { return box_by_tag(e1.box()); }

  template <int Dim>
  const auto &box_by_tag(const Box<Dim> &bx) const {
    return bx;
  }
...
// unary
template <class TExpr>
inline auto abs(const TensorExpr<TExpr> &rhs) {
  return TensorUnaryOp<TExpr, NS4_Ops::absolute<>>(
      static_cast<const TExpr &>(rhs));
}

template <class TExpr>
inline auto operator-(const TensorExpr<TExpr> &rhs) {
...
inline auto operator+(const TensorExpr<TExpr1> &lhs,
                      const TensorExpr<TExpr2> &rhs) {
  return TensorBinaryOp<TExpr1, TExpr2, std::plus<>>(
      static_cast<const TExpr1 &>(lhs),
      static_cast<const TExpr2 &>(rhs));  // downcast to derived class
}

template <class TExpr1>
inline auto operator+(const TensorExpr<TExpr1> &lhs, Real rhs) {
...
inline auto reduce(const TensorExpr<TExpr> &expr,
                   const RedcOp &rop,
                   const TVal &initVal) {
