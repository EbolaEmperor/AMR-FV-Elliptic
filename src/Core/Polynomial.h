...
public:
  ...
  friend Polynomial<Ord1 + Ord2 - 1, CT> operator*(
      const Polynomial<Ord1, CT> &lhs,
      const Polynomial<Ord2, CT> &rhs);
...
inline auto getComp(const Polynomial<Order, CoefType> &poly, int comp) {
  using T = std::decay_t<decltype(poly[0][0])>;  // the additional [0] is to
                                                 // get a component
...
struct _PolynomialRoots<2> {
  template <class Inserter>
  static Inserter roots(const RealPolynomial<2> &poly,
                        Inserter rts,
                        Real tol) {
    *rts++ = poly[0] / poly[1];
    return rts;
...
struct _PolynomialRoots<3> {
  template <class Inserter>
  static Inserter roots(const RealPolynomial<3> &poly,
                        Inserter rts,
                        Real tol) {
    const Real &a = poly[2];
    const Real &b = poly[1];
...
inline Inserter roots(const RealPolynomial<Order> &poly,
                      Inserter rts,
                      Real tol) {
...
template <int Order>
inline Real root(const RealPolynomial<Order> &poly,
                 Real x0,
                 Real tol,
                 int maxIter) {
  auto q = poly.der();
...
template <class Inserter, int Order>
inline Inserter extrema(const RealPolynomial<Order> &poly,
                        Inserter rts,
                        Real tol) {
  if (Order >= 3)
