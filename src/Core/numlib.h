...
template <class T_Func, class T_Der>
inline Real fzero(const T_Func &f,
                  const T_Der &df,
                  Real x0,
                  int maxIter,
                  Real tol) {
  ...
  if (maxIter == 0 && std::abs(fx) > tol)
    dbgcout2 << "Newton iteration may not converge, f(x) = " << fx
             << std::endl;
  return x0;
}
...
struct quad_wrapper {
  template <int numK, class T_Func>
  static Real q(const T_Func &g,
                const Vec<Real, Dim> &lo,
...
inline Real quad(const T_Func &g,
                 const Vec<Real, Dim> &lo,
                 const Vec<Real, Dim> &hi) {
...
inline Real aquad(const T_Func &g, Real a, Real b, Real absTol = 1e-16) {
  ...
  Real R[maxDepth + 1][2];  // [][0] results of trapezoidal rule, [][1]
...
struct GaussLegendreConstant<4> {
  static constexpr Real knots[] = {-0.8611363115940526, -0.3399810435848563};
...
struct GaussLegendreConstant<5> {
  ...
  static constexpr Real weights[] = {0.2369268850561891,
                                     0.4786286704993665,
                                     0.5688888888888889};
};
...
struct GaussLegendreConstant<6> {
  static constexpr Real knots[] = {-0.9324695142031521,
                                   -0.6612093864662645,
                                   -0.2386191860831969};
  static constexpr Real weights[] = {0.1713244923791704,
                                     0.3607615730481386,
                                     0.4679139345726910};
};
...
struct GaussLegendreConstant<7> {
  static constexpr Real knots[] = {-0.9491079123427585,
                                   -0.7415311855993945,
                                   -0.4058451513773972,
                                   0};
  static constexpr Real weights[] = {0.1294849661688697,
                                     0.2797053914892766,
                                     0.3818300505051189,
                                     0.4179591836734694};
};
