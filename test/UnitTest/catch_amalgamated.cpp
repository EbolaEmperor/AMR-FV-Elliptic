...
static sample resample(URng &rng,
...
static sample jackknife(double (*estimator)(double const *, double const *),
                        double *first,
                        double *last) {
...
OutlierClassification classify_outliers(double const *first,
                                        double const *last) {
  std::vector<double> copy(first, last);

  auto q1 =
      weighted_average_quantile(1, 4, copy.data(), copy.data() + copy.size());
  auto q3 =
      weighted_average_quantile(3, 4, copy.data(), copy.data() + copy.size());
  auto iqr = q3 - q1;
  auto los = q1 - (iqr * 3.);
  auto lom = q1 - (iqr * 1.5);
  auto him = q3 + (iqr * 1.5);
...
AssertionResult::AssertionResult(AssertionInfo const &info,
    ...
    m_info(info), m_resultData(CATCH_MOVE(data)) {}
...
static Optional<bazelShardingOptions> readBazelShardingOptions() {
  ...
  if (!has_all) {
    ...
    auto warn = [](const char *env_var) {
      Catch::cerr() << "Warning: Bazel shard configuration is missing '"
  ...
  if (!shardIndex) {
    Catch::cerr() << "Warning: could not parse 'TEST_SHARD_INDEX' ('"
                  << bazelShardIndex << "') as unsigned int.\n";
    return {};
  }
  auto shardTotal = parseUInt(bazelShardTotal);
  if (!shardTotal) {
    Catch::cerr() << "Warning: could not parse 'TEST_TOTAL_SHARD' ('"
...
bool operator==(ProcessedReporterSpec const &lhs,
  ...
  return lhs.name == rhs.name && lhs.outputFilename == rhs.outputFilename &&
...
std::vector<ProcessedReporterSpec> const &Config::getProcessedReporterSpecs()
    const {
...
ColourMode Config::defaultColourMode() const {
...
unsigned int Config::benchmarkSamples() const {
...
IEventListenerPtr createReporter(std::string const &reporterName,
                                 ReporterConfig &&config) {
  auto reporter = Catch::getRegistryHub().getReporterRegistry().create(
      reporterName, CATCH_MOVE(config));
...
IEventListenerPtr prepareReporters(Config const *config) {
  if (Catch::getRegistryHub().getReporterRegistry().getListeners().empty() &&
    ...
    return createReporter(spec.name,
                          ReporterConfig(config,
  ...
  for (auto const &reporterSpec : config->getProcessedReporterSpecs()) {
    multi->addReporter(
        createReporter(reporterSpec.name,
                       ReporterConfig(config,
...
public:
  explicit TestGroup(IEventListenerPtr &&reporter, Config const *config) :
    ...
    m_tests =
        createShard(m_tests, m_config->shardCount(), m_config->shardIndex());
  }
...
Session::Session() {
  ...
  if (!exceptions.empty()) {
    ...
    auto colourImpl =
        makeColourImpl(ColourMode::PlatformDefault, errStream.get());
...
bool applies(TestCaseProperties tcp) {
  static_assert(static_cast<TCP_underlying_type>(TestCaseProperties::None) ==
                    0,
...
bool TestSpec::TagPattern::matches(TestCaseInfo const &testCase) const {
  return std::find(begin(testCase.tags), end(testCase.tags), Tag(m_tag)) !=
         end(testCase.tags);
}
...
std::string convertIntoString(StringRef string) {
  return convertIntoString(string,
...
std::string StringMaker<char *>::convert(
    char *str) {  // NOLINT(readability-non-const-parameter)
  if (str) {
    return Detail::convertIntoString(str);
  } else {
    return {"{null string}"};
...
std::string StringMaker<unsigned long long>::convert(
...
std::map<std::string, std::string> const &ReporterConfig::customOptions()
    const {
...
AssertionStats::AssertionStats(AssertionResult const &_assertionResult,
  ...
  if (assertionResult.hasMessage()) {
    ...
    builder.m_info.message =
        static_cast<std::string>(assertionResult.getMessage());
...
bool CaseInsensitiveLess::operator()(StringRef lhs, StringRef rhs) const {
  return std::lexicographical_compare(
      lhs.begin(), lhs.end(), rhs.begin(), rhs.end(), [](char l, char r) {
  ...
  if (it != itEnd) {
    ...
    if (isOptPrefix(next[0])) {
      ...
      if (delimiterPos != StringRef::npos) {
        ...
        m_tokenBuffer.push_back(
            {TokenType::Argument, next.substr(delimiterPos + 1, next.size())});
      } else {
        if (next[1] != '-' && next.size() > 2) {
          // Combined short args, e.g. "-ab" for "-a -b"
          for (size_t i = 1; i < next.size(); ++i) {
            m_tokenBuffer.push_back({TokenType::Option, next.substr(i, 1)});
          }
        } else {
      ...
      if (m_ref->isFlag()) {
        ...
        if (argToken.type != Detail::TokenType::Argument)
          return Detail::InternalParseResult::runtimeError(
              "Expected argument following " + token.token);
        const auto result =
            valueRef->setValue(static_cast<std::string>(argToken.token));
        if (!result)
          return Detail::InternalParseResult(result);
        if (result.value() == ParseResultType::ShortCircuitAll)
          return Detail::InternalParseResult::ok(
...
Detail::InternalParseResult ExeName::parse(std::string const &,
                                           Detail::TokenStream tokens) const {
  return Detail::InternalParseResult::ok(
      Detail::ParseState(ParseResultType::NoMatch, CATCH_MOVE(tokens)));
}
...
void Parser::writeToStream(std::ostream &os) const {
  ...
  for (auto &cols : rows) {
    auto row =
        TextFlow::Column(CATCH_MOVE(cols.left)).width(optWidth).indent(2) +
        TextFlow::Spacer(4) +
...
Detail::InternalParseResult Parser::parse(std::string const &exeName,
                                          Detail::TokenStream tokens) const {
  struct ParserInfo {
...
Clara::Parser makeCommandLineParser(ConfigData &config) {
  ...
  auto const setWarning = [&](std::string const &warning) {
    ...
    return ParserResult ::runtimeError("Unrecognised warning option: '" +
                                       warning + '\'');
  };
  ...
  auto const setRngSeed = [&](std::string const &seed) {
    ...
    if (!parsedSeed) {
      return ParserResult::runtimeError("Could not parse '" + seed +
                                        "' as seed");
    }
  ...
  auto const setDefaultColourMode = [&](std::string const &colourMode) {
    Optional<ColourMode> maybeMode =
        Catch::Detail::stringToColourMode(toLower(colourMode));
    if (!maybeMode) {
      return ParserResult::runtimeError(
    ...
    if (!isColourImplAvailable(mode)) {
      return ParserResult::runtimeError("colour mode '" + colourMode +
  ...
  auto const setReporter = [&](std::string const &userReporterSpec) {
    ...
    if (!parsed) {
      return ParserResult::runtimeError("Could not parse reporter spec '" +
                                        userReporterSpec + "'");
    }
  ...
  auto const setShardCount = [&](std::string const &shardCount) {
    ...
    if (!parsedCount) {
      return ParserResult::runtimeError("Could not parse '" + shardCount +
                                        "' as shard count");
    }
    if (*parsedCount == 0) {
      return ParserResult::runtimeError("Shard count must be positive");
    }
  ...
  auto const setShardIndex = [&](std::string const &shardIndex) {
    ...
    if (!parsedIndex) {
      return ParserResult::runtimeError("Could not parse '" + shardIndex +
                                        "' as shard index");
    }
  ...
  auto cli =
      ...
      Opt(config.defaultOutputFilename,
          "filename")["-o"]["--out"]("default output filename") |
      Opt(accept_many, setReporter, "name[::key=value]*")["-r"]["--reporter"](
      ...
      Opt(config.listTests)["--list-tests"]("list all/matching test cases") |
      Opt(config.listTags)["--list-tags"]("list all/matching tags") |
      Opt(config.listReporters)["--list-reporters"](
          "list all available reporters") |
      Opt(config.listListeners)["--list-listeners"]("list all listeners") |
      Opt(setTestOrder,
          "decl|lex|rand")["--order"]("test case order (defaults to decl)") |
      Opt(setRngSeed, "'time'|'random-device'|number")["--rng-seed"](
          "set a specific seed for random numbers") |
      Opt(setDefaultColourMode, "ansi|win32|none|default")["--colour-mode"](
      ...
      Opt(config.skipBenchmarks)["--skip-benchmarks"](
      ...
      Opt(config.benchmarkWarmupTime,
          ...
          "(default: 100)") |
      Opt(setShardCount, "shard count")["--shard-count"](
          "split the tests to execute into this many groups") |
      Opt(setShardIndex, "shard index")["--shard-index"](
          "index of the group of tests to execute (see --shard-count)") |
      Opt(config.allowZeroTests)["--allow-running-no-tests"](
...
ColourImpl::ColourGuard &ColourImpl::ColourGuard::operator=(
    ColourGuard &&rhs) noexcept {
  using std::swap;
...
ColourImpl::ColourGuard &ColourImpl::ColourGuard::engage(
    std::ostream &stream) & {
  engageImpl(stream);
  return *this;
}
...
Detail::unique_ptr<ColourImpl> makeColourImpl(ColourMode colourSelection,
  ...
  CATCH_ERROR("Could not create colour impl for selection "
              << static_cast<int>(colourSelection));
}
...
void Context::createContext() { currentContext = new Context(); }
...
void ITransientExpression::streamReconstructedExpression(
  ...
  os << "Some class derived from ITransientExpression without overriding "
...
/** \file
 * This file provides platform specific implementations of
...
topLevelExceptionFilter(PEXCEPTION_POINTERS ExceptionInfo) {
...
void FatalConditionHandler::engage_platform() {
  ...
  previousTopLevelExceptionFilter =
      SetUnhandledExceptionFilter(topLevelExceptionFilter);
}

void FatalConditionHandler::disengage_platform() noexcept {
  if (SetUnhandledExceptionFilter(previousTopLevelExceptionFilter) !=
...
void listTags(IEventListener &reporter, IConfig const &config) {
  ...
  for (auto const &testCase : matchedTestCases) {
    for (auto const &tagName : testCase.getTestCaseInfo().tags) {
      ...
      if (it == tagCounts.end())
        it = tagCounts.insert(std::make_pair(tagName.original, TagInfo()))
                 .first;
...
IEventListenerPtr ReporterRegistry::create(std::string const &name,
...
std::vector<std::string> splitReporterSpec(StringRef reporterSpec) {
  ...
  while (separatorPos < reporterSpec.size()) {
    ...
    parts.push_back(static_cast<std::string>(
        reporterSpec.substr(separatorPos, nextSeparator - separatorPos)));
...
ReporterSpec::ReporterSpec(std::string name,
                           Optional<std::string> outputFileName,
                           Optional<ColourMode> colourMode,
                           std::map<std::string, std::string> customOptions) :
...
struct GeneratorTracker final : TestCaseTracking::TrackerBase,
  ...
  GeneratorTracker(TestCaseTracking::NameAndLocation &&nameAndLocation,
                   TrackerContext &ctx,
                   ITracker *parent) :
  ...
  static GeneratorTracker *acquire(
      TrackerContext &ctx,
      TestCaseTracking::NameAndLocationRef const &nameAndLocation) {
  ...
  void close() override {
    ...
    const bool should_wait_for_child = [&]() {
      ...
      auto const &parentSection = static_cast<SectionTracker const &>(*parent);
      ...
      for (auto const &child : m_children) {
        if (child->isSectionTracker() &&
            std::find(
                filters.begin(),
                filters.end(),
                static_cast<SectionTracker const &>(*child).trimmedName()) !=
                filters.end()) {
          return true;
        }
      }
    ...
    if (should_wait_for_child ||
        (m_runState == CompletedSuccessfully && m_generator->countedNext())) {
      m_children.clear();
...
Totals RunContext::runTest(TestCaseHandle const &testCase) {
  ...
  do {
    ...
    auto statsForOneRun = TestCaseStats(testInfo,
                                        singleRunTotals,
                                        CATCH_MOVE(oneRunCout),
                                        CATCH_MOVE(oneRunCerr),
                                        aborting());
...
bool RunContext::sectionStarted(StringRef sectionName,
  ...
  SectionInfo sectionInfo(sectionLineInfo,
                          static_cast<std::string>(sectionName));
...
IGeneratorTracker *RunContext::acquireGeneratorTracker(
    StringRef generatorName,
    SourceLineInfo const &lineInfo) {
  using namespace Generators;
  GeneratorTracker *tracker = GeneratorTracker::acquire(
      m_trackerContext,
      TestCaseTracking::NameAndLocationRef(generatorName, lineInfo));
...
IGeneratorTracker *RunContext::createGeneratorTracker(
  ...
  auto nameAndLoc = TestCaseTracking::NameAndLocation(
...
void RunContext::sectionEnded(SectionEndInfo &&endInfo) {
  ...
  m_reporter->sectionEnded(SectionStats(CATCH_MOVE(endInfo.sectionInfo),
                                        assertions,
...
void RunContext::handleFatalErrorCondition(StringRef message) {
  ...
  SectionStats testCaseSectionStats(
      CATCH_MOVE(testCaseSection), assertions, 0, false);
...
void RunContext::runCurrentTest(std::string &redirectedCout,
  ...
  SectionStats testCaseSectionStats(
      CATCH_MOVE(testCaseSection), assertions, duration, missingAssertions);
  m_reporter->sectionEnded(testCaseSectionStats);
}
...
Section::~Section() {
  if (m_sectionIncluded) {
    SectionEndInfo endInfo{
        CATCH_MOVE(m_info), m_assertions, m_timer.getElapsedSeconds()};
    if (uncaught_exceptions()) {
...
static void enforceNoDuplicateTestCases(
  ...
  auto testInfoCmp = [](TestCaseInfo const *lhs, TestCaseInfo const *rhs) {
    return *lhs < *rhs;
  };
...
NameAndLocation::NameAndLocation(std::string &&_name,
                                 SourceLineInfo const &_location) :
    name(CATCH_MOVE(_name)), location(_location) {}
...
ITracker *ITracker::findChild(NameAndLocationRef const &nameAndLocation) {
  auto it = std::find_if(
      ...
      [&nameAndLocation](ITrackerPtr const &tracker) {
        ...
        if (tnameAndLoc.location.line != nameAndLocation.location.line) {
          return false;
        }
...
TrackerBase::TrackerBase(NameAndLocation &&nameAndLocation,
                         TrackerContext &ctx,
                         ITracker *parent) :
    ITracker(CATCH_MOVE(nameAndLocation), parent), m_ctx(ctx) {}
...
SectionTracker::SectionTracker(NameAndLocation &&nameAndLocation,
                               TrackerContext &ctx,
                               ITracker *parent) :
    TrackerBase(CATCH_MOVE(nameAndLocation), ctx, parent),
...
SectionTracker &SectionTracker::acquire(
    TrackerContext &ctx,
    NameAndLocationRef const &nameAndLocation) {
...
static StringRef extractClassName(StringRef classOrMethodName) {
  ...
  return methodName.substr(static_cast<std::size_t>(startIdx),
...
std::string Column::const_iterator::addIndentAndSuffix(size_t position,
                                                       size_t length) const {
  std::string ret;
...
XmlWriter::ScopedElement &XmlWriter::ScopedElement::writeAttribute(
    StringRef name,
    StringRef attribute) {
...
std::string WithinUlpsMatcher::describe() const {
  ...
  if (m_type == Detail::FloatingPointKind::Double) {
    write(ret,
          step(m_target, -std::numeric_limits<double>::infinity(), m_ulps));
    ret << ", ";
    write(ret,
          step(m_target, std::numeric_limits<double>::infinity(), m_ulps));
  } else {
...
WithinUlpsMatcher WithinULP(double target, uint64_t maxUlpDiff) {
  return WithinUlpsMatcher(
      target, maxUlpDiff, Detail::FloatingPointKind::Double);
}

WithinUlpsMatcher WithinULP(float target, uint64_t maxUlpDiff) {
  return WithinUlpsMatcher(
      target, maxUlpDiff, Detail::FloatingPointKind::Float);
}
...
StringMatcherBase::StringMatcherBase(StringRef operation,
...
StringEqualsMatcher::StringEqualsMatcher(CasedString const &comparator) :
    StringMatcherBase("equals"_sr, comparator) {}
...
StringContainsMatcher::StringContainsMatcher(CasedString const &comparator) :
    StringMatcherBase("contains"_sr, comparator) {}
...
StartsWithMatcher::StartsWithMatcher(CasedString const &comparator) :
    StringMatcherBase("starts with"_sr, comparator) {}
...
EndsWithMatcher::EndsWithMatcher(CasedString const &comparator) :
    StringMatcherBase("ends with"_sr, comparator) {}
...
RegexMatcher::RegexMatcher(std::string regex, CaseSensitive caseSensitivity) :
    m_regex(CATCH_MOVE(regex)), m_caseSensitivity(caseSensitivity) {}
...
StringContainsMatcher ContainsSubstring(std::string const &str,
                                        CaseSensitive caseSensitivity) {
...
std::string describe_multi_matcher(StringRef combine,
  ...
  combined_size +=
      static_cast<size_t>(descriptions_end - descriptions_begin - 1) *
      combine.size();
...
void handleExceptionMatchExpr(AssertionHandler &handler,
  ...
  MatchExpr<std::string, StringMatcher const &> expr(
      CATCH_MOVE(exceptionMessage), matcher);
...
private:
  ...
  void printExpressionWas() {
    if (result.hasExpression()) {
      stream << ';';
      {
        stream << colourImpl->guardColour(compactDimColour)
               << " expression was:";
      }
      printOriginalExpression();
    }
...
void CompactReporter::assertionEnded(AssertionStats const &_assertionStats) {
  ...
  AssertionPrinter printer(
      m_stream, _assertionStats, printInfoMessages, m_colour.get());
  printer.print();
...
void CompactReporter::sectionEnded(SectionStats const &_sectionStats) {
  ...
  if (shouldShowDuration(*m_config, dur)) {
    m_stream << getFormattedDuration(dur)
  ...
  void printReconstructedExpression() const {
    if (result.hasExpandedExpression()) {
      ...
      stream << colourImpl->guardColour(Colour::ReconstructedExpression)
             << TextFlow::Column(result.getExpandedExpression()).indent(2)
             << '\n';
    }
  }
...
std::size_t &findMax(std::size_t &i,
                     std::size_t &j,
                     std::size_t &k,
                     std::size_t &l) {
  if (i > j && i > k && i > l)
...
public:
  TablePrinter(std::ostream &os, std::vector<ColumnInfo> columnInfos) :
      m_os(os), m_columnInfos(CATCH_MOVE(columnInfos)) {}
...
ConsoleReporter::ConsoleReporter(ReporterConfig &&config) :
    ...
    m_tablePrinter(Detail::make_unique<TablePrinter>(
        m_stream,
          ...
          if (config.fullConfig()->benchmarkNoAnalysis()) {
            ...
            return {
                ...
                {"iterations   low mean   low std dev",
                 14,
                 Justification::Right},
                {"est run time high mean  high std dev",
                 14,
                 Justification::Right}};
          }
        }())) {}
...
void ConsoleReporter::assertionEnded(AssertionStats const &_assertionStats) {
  ...
  ConsoleAssertionPrinter printer(
      m_stream, _assertionStats, m_colour.get(), includeResults);
  printer.print();
  m_stream << '\n' << std::flush;
}
...
void ConsoleReporter::printHeaderString(std::string const &_string,
  ...
  m_stream
      << TextFlow::Column(_string).indent(indent + idx).initialIndent(indent)
      << '\n';
}
...
struct BySectionInfo {
  ...
  bool operator()(Detail::unique_ptr<CumulativeReporterBase::SectionNode> const
                      &node) const {
...
void CumulativeReporterBase::sectionStarting(SectionInfo const &sectionInfo) {
  ...
  if (m_sectionStack.empty()) {
    if (!m_rootSection) {
      m_rootSection = Detail::make_unique<SectionNode>(incompleteStats);
    }
...
bool CumulativeReporterBase::SectionNode::hasAnyAssertions() const {
  return std::any_of(assertionsAndBenchmarks.begin(),
...
void defaultListReporters(std::ostream &out,
                          std::vector<ReporterDescription> const &descriptions,
                          Verbosity verbosity) {
...
void defaultListListeners(
  ...
  const auto maxNameLen =
      std::max_element(
          descriptions.begin(),
          descriptions.end(),
          [](ListenerDescription const &lhs, ListenerDescription const &rhs) {
            return lhs.name.size() < rhs.name.size();
          })
          ->name.size();
...
void defaultListTests(std::ostream &out,
  ...
  for (auto const &test : tests) {
    ...
    if (verbosity >= Verbosity::High) {
      out << TextFlow::Column(Catch::Detail::stringify(testCaseInfo.lineInfo))
                 .indent(4)
          << '\n';
    }
...
void printTestRunTotals(std::ostream &stream,
  ...
  if (totals.assertions.total() > 0 && totals.testCases.allPassed()) {
    stream << streamColour.guardColour(Colour::ResultSuccess)
           << "All tests passed";
    stream << " (" << pluralise(totals.assertions.passed, "assertion"_sr)
           << " in " << pluralise(totals.testCases.passed, "test case"_sr)
           << ')' << '\n';
    return;
  }
...
JsonReporter::JsonReporter(ReporterConfig &&config) :
...

  {
    ...
    if (m_config->testSpec().hasFilters()) {
      metadata_writer.write("filters"_sr).write(m_config->testSpec());
    }
  }
}
...
void JsonReporter::testCaseEnded(TestCaseStats const &tcStats) {
...

  {
    ...
    // We do not write the test case totals, because there will always be just
    // one test case here.
    // TODO: overall "result" -> success, skip, fail here? Or in partial
    // result?
  }
  // We do not write out stderr/stdout, because we instead wrote those out in
  // partial runs
...
void JsonReporter::testCasePartialEnded(TestCaseStats const &tcStats,
  ...
  if (!tcStats.stdOut.empty()) {
    m_objectWriters.top().write("captured-stdout"_sr).write(tcStats.stdOut);
  }
  if (!tcStats.stdErr.empty()) {
    m_objectWriters.top().write("captured-stderr"_sr).write(tcStats.stdErr);
  }
  {
    auto totals = m_objectWriters.top().write("totals"_sr).writeObject();
    writeCounts(totals.write("assertions"_sr).writeObject(),
...
JunitReporter::JunitReporter(ReporterConfig &&_config) :
    CumulativeReporterBase(CATCH_MOVE(_config)), xml(m_stream) {
  m_preferences.shouldRedirectStdOut = true;
  m_preferences.shouldReportAllAssertions = true;
...
void JunitReporter::writeRun(TestRunNode const &testRunNode,
                             double suiteTime) {
  ...
  xml.writeAttribute("failures"_sr,
...
void JunitReporter::writeSection(std::string const &className,
  ...
  if (sectionNode.hasAnyAssertions() || !sectionNode.stdOut.empty() ||
...
void MultiReporter::addListener(IEventListenerPtr &&listener) {
  ...
  m_reporterLikes.insert(m_reporterLikes.begin() + m_insertedListeners,
                         CATCH_MOVE(listener));
  ++m_insertedListeners;
}
...
void MultiReporter::addReporter(IEventListenerPtr &&reporter) {
  ...
  m_haveNoncapturingReporters |=
      !reporter->getPreferences().shouldRedirectStdOut;
...
void MultiReporter::testCasePartialStarting(TestCaseInfo const &testInfo,
                                            uint64_t partNumber) {
  for (auto &reporterish : m_reporterLikes) {
...
void MultiReporter::listReporters(
    std::vector<ReporterDescription> const &descriptions) {
  for (auto &reporterish : m_reporterLikes) {
...
void registerListenerImpl(
    Detail::unique_ptr<EventListenerFactory> listenerFactory) {
...
void TAPReporter::assertionEnded(AssertionStats const &_assertionStats) {
  ...
  TapAssertionPrinter printer(
      m_stream, _assertionStats, counter, m_colour.get());
  printer.print();
...
XmlReporter::XmlReporter(ReporterConfig &&_config) :
    StreamingReporterBase(CATCH_MOVE(_config)), m_xml(m_stream) {
  m_preferences.shouldRedirectStdOut = true;
  m_preferences.shouldReportAllAssertions = true;
}
...
void XmlReporter::sectionStarting(SectionInfo const &sectionInfo) {
  ...
  if (m_sectionDepth++ > 0) {
    m_xml.startElement("Section").writeAttribute(
        "name"_sr, trim(StringRef(sectionInfo.name)));
  ...
  if (--m_sectionDepth > 0) {
    {
      ...
      e.writeAttribute("expectedFailures"_sr,
...
void XmlReporter::testRunEnded(TestRunStats const &testRunStats) {
  ...
  m_xml.scopedElement("OverallResults")
      ...
      .writeAttribute("expectedFailures"_sr,
...
void XmlReporter::benchmarkPreparing(StringRef name) {
  m_xml.startElement("BenchmarkResults").writeAttribute("name"_sr, name);
}
...
void XmlReporter::listListeners(
    std::vector<ListenerDescription> const &descriptions) {
